export class CheckoutOptionsComponent {
    ...
    selectRegisterAccount() {
        const registerAccountRadio = this.root.$('input[type="radio"][value="register"]');
        expect(registerAccountRadio).toBeClickable({ message: 'Expected register account radio button to be visible. Make sure you are logged in' })
